class GradsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|  
      if @grad.save
        format.html { redirect_to grads_path(:university => @grad.university), notice: 'Новый выпускник сохранился :).' }
        format.json { render :show, status: :created, location: @grad }
      else
        format.html { redirect_to new_grad_path(:university => @grad.university) }
  ...
  def destroy
    @grad.destroy
    respond_to do |format|
      format.html { redirect_to grads_path(:university => @grad.university), notice: 'Grad was successfully destroyed.' }
  ...
  private
    ...
    def grad_params
      params.require(:grad).permit(:fio, :iin, :dob, :otd, :forma, :gender, :mob, :university)
    end
end
